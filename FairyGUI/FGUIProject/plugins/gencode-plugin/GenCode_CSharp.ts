...
function genCode(handler: FairyEditor.PublishHandler) {
    ...
    for (let i: number = 0; i < classCnt; i++) {
        ...
        for (let j: number = 0; j < memberCnt; j++) {
            ...
            if (memberInfo.res != null && memberInfo.type == "GComponent") {
                // console.log(d + ":" + memberInfo.type + "-" + memberInfo.res.fileName + "-" + memberInfo.res.type + "-" + memberInfo.res.name)
                isCustomComponent = true;

            }


            writer.writeln('public %s %s;', isCustomComponent ? memberInfo.res.name : memberInfo.type, memberInfo.varName);
        }
        ...
        for (let j: number = 0; j < memberCnt; j++) {
            ...
            // console.log("res=" + memberInfo.res)
            ...
            if (memberInfo.group == 0) {
                ...
                if (getMemberByName) {
                    writer.writeln('%s = (%s)GetChild("%s");', memberInfo.varName, isCustomComponent ? memberInfo.res.name : memberInfo.type, memberInfo.name);
                }
                else {
                    writer.writeln('%s = (%s)GetChildAt(%s);', memberInfo.varName, isCustomComponent ? memberInfo.res.name : memberInfo.type, memberInfo.index);
                }
            }
